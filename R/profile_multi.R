profile.multi <- function(iter = 0, max.cond = TRUE, marginal = list(value = Inf), cond = list(value = Inf),
                          X, cond.scale, K, v, X.tilde,X.tilde_C,slope,
                          tr, y, d, stratum, z,  beta, theta,slope.model,lnphipsi,lnpi,N,
                          scale.cond,dt_1, tr.dt_1, m_1,cum.numh.p, numh.p, num.lam.p, breaks,p.strata,
                          Q.partial,n, n.p, partial.stratum,
                          num.a, num.g, d.marg, tr.idx,n_C, n_i, max.n_i, sum.d_i,
                          ag.idx, a.idx, g.idx, IG.idx, PVF.idx,  Addams.idx,aNBp.idx,aNB.idx,cP.idx,H.idx,
                          ag_idx, a_idx, g_idx, IG_idx, PVF_idx, J,
                          idx, frailty, alpha.idx, gamma.idx, fac,initial.z,start_dist,
  ...
  while(max.cond){
    ...
    cond <- optimax(par = beta, fn = partial.loglihood, gr = dpartial.db, Hessian = c.Hessian,
                    options = c.options, Richardson = FALSE, R.options = NULL, method = c.method,
    ...
    if(!slope){
      ...
      marginal <- optimax(par = c(theta,lnpi,lnphipsi), fn = slope.opt, gr = m.gr, Hessian = m.Hessian,
                          Richardson = Richardson, R.options = R.options, options = m.options, method = m.method,
                          Lambda_i=Lambda_i,
    ...
    if(!slope){
      z <- z.multi(alpha = alpha, gamma = gamma, ag_i = ag_idx, g_i = g_idx, a_i = a_idx, PVF_i = PVF_idx,
                   s = s, frailty = frailty, n = n_C, ID = ID, sum.d_i = sum.d_i, iter = iter, fac = fac)$z
    }else{
    ...
    if(prnt) {print(model_par); print(paste('profile: marginal loglihood = ', marginal$value))}
    if(iter == iterlim & max.cond){iter <- iter+1; break}
  }

  return(list(marginal = marginal, cond = cond, iter = iter, beta = beta, uni.logli=uni.logli,
              diff.cond = diff.cond, diff.marginal = diff.marginal, z = z, v = v, lnpi=lnpi, pi = pi, v.Domain = v.Domain,
              lambda_i = lambda_i, theta = theta, model_par = model_par, Lambda_i = Lambda_i, lnphipsi = lnphipsi,
